...
public class PHIGUIXEDao implements DAOInterface<PHIGUIXE> {
	...
	public ArrayList<Map<String, Object>> selectAllWithDetails() {
		String sql = "SELECT PGX.CCCDchuho, PGX.Soxemay, PGX.Sooto, PGX.Thoihan, HGD.Hotenchuho AS Tenchuho, CH.Idcanho, CH.Sonha FROM PHIGUIXE PGX JOIN HOGIADINH HGD ON PGX.CCCDchuho = HGD.CCCDchuho JOIN CANHO CH ON HGD.Idcanho = CH.Idcanho";
		...
		try (Connection conn = DatabaseUtil.getConnection();
			...
			while (rs.next()) {
				...
				row.put("CCCDchuho", rs.getString("CCCDchuho"));
				row.put("Soxemay", rs.getInt("Soxemay"));
				row.put("Sooto", rs.getInt("Sooto"));
				row.put("Thoihan", rs.getDate("Thoihan"));
			...
			throw new RuntimeException("Lỗi khi lấy thông tin PHIGUIXE với chi tiết", e);
		}
